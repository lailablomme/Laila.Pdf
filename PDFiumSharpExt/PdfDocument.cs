...
namespace PDFiumSharp
{
    public sealed class PdfDocument : NativeWrapper<FPDF_DOCUMENT>
    {
		...
		/// </summary>
		public PdfPageCollection Pages { get; private set; }
		...
		public bool Save(Stream stream, SaveFlags flags = SaveFlags.None, int version = 0)
		{
			try
			{
                PDFium.FORM_DoDocumentAAction(this.FormFillEnvironment.Handle, 0x11);
                return PDFium.FPDF_SaveAsCopy(Handle, stream, flags, version);
			}
			finally
			{
				PDFium.FORM_DoDocumentAAction(this.FormFillEnvironment.Handle, 0x12);
            }
        }
		...
		protected override void Dispose(FPDF_DOCUMENT handle)
		{
            ...
            if (_formFillEnvironment != null)
                ((IDisposable)_formFillEnvironment).Dispose();
        }

        private PdfFormFillEnvironment? _formFillEnvironment = null;

        public PdfFormFillEnvironment FormFillEnvironment
        {
            get
            {
                if (_formFillEnvironment == null)
                    _formFillEnvironment =	new PdfFormFillEnvironment(this);
                return _formFillEnvironment;
            }
        }
    }
}
