...
namespace PDFiumSharp
{
    public sealed class PdfPage : NativeWrapper<FPDF_PAGE>
    {
        ...
        /// <summary>
        /// Renders the form to a <see cref="PDFiumBitmap"/>
        /// </summary>
        /// <param name="renderTarget">The bitmap to which the page is to be rendered.</param>
        /// <param name="rectDest">The destination rectangle in <paramref name="renderTarget"/>.</param>
        /// <param name="orientation">The orientation at which the page is to be rendered.</param>
        /// <param name="flags">The flags specifying how the page is to be rendered.</param>
        public void RenderForm(PDFiumBitmap renderTarget, (int left, int top, int width, int height) rectDest, PageOrientations orientation = PageOrientations.Normal, RenderingFlags flags = RenderingFlags.None)
        {
            if (renderTarget == null)
                throw new ArgumentNullException(nameof(renderTarget));

            PDFium.FPDF_FFLDraw(this.Document.FormFillEnvironment.Handle, renderTarget.Handle, this.Handle, rectDest.left, rectDest.top, rectDest.width, rectDest.height, (int)orientation, (int)flags);
        }
        ...
        private PdfTextPage? _textPage = null;

        public PdfTextPage TextPage
        {
            get
            {
                if (_textPage == null)
                    _textPage = PdfTextPage.Load(this);
                return _textPage;
            }
        }

        private PdfForm? _form = null;
        ...
        public PdfForm Form
        {
            ...
            {
                if (_form == null)
                    _form = new PdfForm(this.Document, this);
                return _form;
            }
        }
    }
